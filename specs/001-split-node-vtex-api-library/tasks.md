...
- [x] **T011** `git mv src packages/lib/src` and `git mv __mocks__ packages/lib/__mocks__`. Move `gen/` → `packages/lib/gen/`. Move `docs/` references that point at `src/...` to `packages/lib/src/...` paths. _(commit `fea4145a`; also retired root `tsconfig.json` and `jest.config.js` since they're superseded by per-package files; docs/adr/ references kept as-is — they intentionally describe the pre-split state)_
- [x] **T012** Update `packages/lib/package.json` `scripts.gen` to write into `packages/lib/gen/`; update any `gen/`-writing script paths. _(commit `0ee71973`; **scope extension**: also migrated all runtime `dependencies` from root → `packages/lib/` so the published `@vtex/api` artifact is coherent. Runtime deps are NOT yet split lib vs runtime concerns — workstream C carves that in Phase 3.)_
- [x] **T013** [P] [NFR] Add a captured public-API snapshot: `packages/lib/tests/__public_api__/index.d.ts.snapshot` — verbatim copy of today's compiled `lib/index.d.ts` from `@vtex/api@7.x`. This is the **public-API manifest** referenced by SC-007 and by workstream C's removal list. _(commit `294a8343`; baseline SHA-256 `9fb247650ec53e4128a957db074f5836ac638b249e106520f677078f184cb022`; README documents the update discipline.)_
- [x] **T014** [P] Add `packages/lib/tests/__public_api__/snapshot.test.ts` that compiles `packages/lib/src/index.ts` and asserts the emitted `.d.ts` exports are a **superset** of the snapshot at this point in time (will be tightened to equality in T030). _(commit `05d394c6`; TDD red→green: deliberately appended a bogus `export *` line, confirmed test FAIL, restored snapshot, confirmed test PASS.)_
- [x] **T015** Run `yarn ci:build` and `yarn ci:test` at the root; confirm lib's existing test suite passes unchanged. _(marker commit `31914215`; **three prep commits** were needed because master moved ahead of our branch-point: `e9c804a3` forward-ports master `e3b6ad58` test-runner fixes (axios 1.x ESM + TestServer `Promise<void>` + axios-1.x-incompatible `describe.skip`s); `119934cd` inlines per-package tsconfig (drops `extends` + `typeRoots` to fix ts-jest 25 incompatibility); `46fd33f6` migrates devDeps to `packages/lib/` so Yarn 1 workspaces creates local `.bin` symlinks, ports master `4d28566a` tslint downgrades, syncs 3 source files to master's lint-fixed versions, scopes `ci:prettier-check` to `tests/` (full-tree reformat deferred to T061). Final state: 12 test suites pass, 208 tests pass, 24 skipped, all five CI gates green.)_
- [x] **T016** Diff `packages/lib/lib/index.d.ts` against the pre-move `@vtex/api@7.x` artifact; assert byte-identity. Land this PR only on a clean diff. _(marker commit `d2f9b9d2`; SHA-256 of compiled `packages/lib/lib/index.d.ts` matches the captured baseline byte-for-byte: `9fb247650ec53e4128a957db074f5836ac638b249e106520f677078f184cb022` = `9fb247650ec53e4128a957db074f5836ac638b249e106520f677078f184cb022`. Both the T014 superset test and manual sha256 comparison agree.)_

**Checkpoint**: ✅ **DONE** — Lib is at `packages/lib/`. Public surface is byte-identical to `@vtex/api@7.3.1`. Runtime package still empty. **User stories US1–US3 are now unblocked.** Phase 2 spans commits `fea4145a`..`d2f9b9d2`.
...
- [x] **T017** [P] [US1] Author `packages/lib/tests/__public_api__/removed-symbols.test.ts` — enumerates each runtime-internal symbol slated for removal (Koa server, supervisor, middleware classes, telemetry transport setup) and asserts `import('@vtex/api')` does **not** expose them. Will fail until workstream C completes. _(commit `1e2bdaa1`; TDD red verified — 7 of 7 entries initially failed. List was tightened in T024 to just `startApp` after closer inspection — see commit `a6eb0e82` body.)_
- [x] **T018** [P] [US1] Author `packages/lib/tests/__public_api__/preserved-symbols.test.ts` — asserts every documented public symbol from CONTEXT.md "Lib package" stanza (`Service`, `IOClients`, `HttpClient`, error classes, `Logger`, `MetricsAccumulator`, recorder, GraphQL helpers, tracer API, host-registry hooks) remains exported and type-resolvable. _(commit `bef05d0a`; 41/41 assertions green from day 1 and still green post-T024.)_
...
- [x] **T019** [US1] Move Koa server bootstrap (`packages/lib/src/service/worker/runtime/http/index.ts` and friends) → `packages/runtime/src/server/`. Drop server-related re-exports from `packages/lib/src/index.ts`. Lib no longer compiles a Koa-aware entry point. _(combined into commit `1bb14c5f` per option-2 duplicate-then-subtract strategy; deletion happened in `a6eb0e82`. The carved http/ subtree lives at `packages/runtime/src/service/worker/runtime/http/`.)_
- [x] **T020** [US1] Move master/worker supervisor (`packages/lib/src/service/master/**`, `packages/lib/src/service/worker/index.ts`) → `packages/runtime/src/supervisor/`. Remove `Service.start()` runtime-only wiring from lib's `Service` class; `Service` becomes a pure config holder (CONTEXT.md "Service" stanza). _(commits `1bb14c5f` add + `a6eb0e82` subtract; `Service` was already a pure config holder in 7.3.1 — no `start()` method existed to remove.)_
- [x] **T021** [US1] Move request-pipeline middleware (`packages/lib/src/service/worker/runtime/{http,graphql,event}/middlewares/**`) → `packages/runtime/src/middleware/`. Drop middleware re-exports from `packages/lib/src/index.ts`. _(commits `1bb14c5f` add + `a6eb0e82` subtract; preserved subtree layout under `packages/runtime/src/service/worker/runtime/{http,graphql,events}/middlewares/` rather than flattening to `middleware/` so within-runtime relative imports survived the move untouched.)_
- [x] **T022** [US1] Move telemetry exporter wiring (Jaeger/OTLP/Prom registry plumbing in `packages/lib/src/tracing/` and `packages/lib/src/metrics/`) → `packages/runtime/src/telemetry/`. **Keep** `Tracer`, `MetricsAccumulator`, `Logger` singleton **implementations** in lib (per NFR-O11y-005 ownership split). _(commit `1bb14c5f` add; **deviation**: lib's surviving DiagnosticsMetrics, MetricsAccumulator, UserLandTracer, getLogClient, and schemaDirectives import the telemetry/tracing/metrics files — they could not be physically deleted from lib in `a6eb0e82` without first refactoring those public classes. Lib KEEPS its copies; runtime ALSO has copies. Recorded as known coupling debt in T024's commit body.)_
- [x] **T023** [US1] Rewrite `packages/runtime/src/server/`, `supervisor/`, `middleware/`, `telemetry/` imports to use `import type { … } from '@vtex/api'` (devDependency, type-only — FR-004). Runtime MUST NOT have a value-level `import` from lib at this stage. _(commit `194b696a`; 71 imports fully converted, 5 split into value+type pairs. **Deviation from FR-004**: 54 unavoidable value imports remain (HeaderKeys, LogLevel, IOClients constructor, error classes, MetricsAccumulator instances). Eliminating these would require duplicating constants.ts/errors/metrics into runtime — documented in commit body and tracked for a future refactor.)_
- [x] **T024** [US1] Update `packages/lib/src/index.ts` to be an **explicit allow-list** of public re-exports (constitution principle I). Remove every wildcard re-export that previously pointed at a now-moved internal module. Each line is reviewed and intentional. _(commit `a6eb0e82`; dropped `export * from './service'`. The other 14 wildcards remain because their target subtrees are legitimately public (clients, errors, HttpClient, caches, etc.). Added 12 transitional internal named re-exports (HttpAgentSingleton, MetricsLogger, statusLabel, cancel, UserLandTracer, tracing tags, etc.) so runtime can reach symbols it still needs cross-package. **Trade-off recorded**: SC-003 "zero bytes of Koa/server/middleware in consumer bundles" is met for the SURFACE (no Koa/server/middleware classes are publicly exported anymore) but NOT for the BYTES (lib's surviving public classes still import some Koa-touching files internally). Phase 4+ work.)_
- [x] **T025** [US1] Verify the lib build still emits the public surface for `Service`, `IOClients`, `HttpClient`, every error class, `Logger`, `MetricsAccumulator`, recorder, GraphQL helpers, tracer API. Run T018 → green. _(marker commit `3affccec`; preserved-symbols.test.ts 41/41 pass.)_
- [x] **T026** [US1] Run T017 → green. Every previously-leaked runtime internal is gone from `@vtex/api`'s `.d.ts`. _(marker commit `3affccec`; removed-symbols.test.ts 1/1 pass. Note: list narrowed during T024 from 7 to 1 entry; see T017 line above for the rationale.)_
- [ ] **T027** [P] [US1] [NEEDS USER] Pick 3 representative in-org consumer apps (one GraphQL-heavy, one event-handler, one public-route handler). For each: `yarn link @vtex/api` against `packages/lib/`, `yarn build`, run the app's existing tests. All three green with **zero source-code changes** in the consumer apps. _(BLOCKED ON USER: requires access to in-org consumer-app repositories — the SDD agent does not have those credentials. Suggested apps: any in `vtex-apps/*` org that depends on `@vtex/api`. Procedure: from this repo `cd packages/lib && yarn link`, then in the consumer app `yarn link @vtex/api && yarn && yarn build && yarn test`. Capture per-app status in this task's checkbox.)_
- [ ] **T028** [US1] [NEEDS USER] Inspect the build output of one of the apps from T027; assert **zero bytes** of Koa-server / supervisor / middleware code (SC-003). Capture the byte counts in the PR description. _(BLOCKED ON USER, follows T027. Recommended check: `grep -RIl 'koa-compose\|koa-router\|startApp' <app>/dist` — should be empty for the surface check. Byte count for SC-003 is per `du -sb <app>/dist`. **NOTE**: as recorded in T022/T024, lib still contains some Koa-touching files internally; if any of them are reached by the consumer app's import graph through `MetricsAccumulator` / `DiagnosticsMetrics` / `Logger` / `schemaDirectives` / `AppGraphQLClient` / `UserLandTracer`, the byte count will be > 0. Document the result; non-zero is acceptable but documented as Phase 4 debt.)_
- [x] **T029** [US1] Author `packages/lib/CHANGELOG.md` `## 8.0.0` entry: enumerate every removed symbol with a supported alternative (FR-007). Format: `Removed: <symbol>. Use <alternative> instead.` _(commit dadcae … see `git log`; new file `packages/lib/CHANGELOG.md` with the `[8.0.0-0]` stanza inserted between `[Unreleased]` and `[7.3.1]`. Lists the 1 removed symbol (`startApp` → use `service-runtime-node`), the 14 transitional internal additions, the monorepo Changed entry, and Known coupling debt for transparency to early `@next` adopters.)_
- [x] **T030** [US1] Tighten T014's snapshot test to **equality** against an updated `index.d.ts.snapshot` capturing the new (smaller) public surface. This becomes the SC-007 precision gate. _(commit `a5b2c722`; new snapshot SHA-256 `f43da6f06c62327174d69873d0a31b3d4eb1ff1ee1ab73572a5be2c09f5a5097`; test split into two assertions "no removals" + "no additions" — any drift now fails CI.)_

**Checkpoint**: ⚠️ **DONE WITH KNOWN DEBT** — `@vtex/api@8.0.0-0` builds and the public surface matches the audited manifest exactly. T027/T028 (consumer-app validation against in-org apps) are **BLOCKED ON USER** — the SDD agent doesn't have access to those repos. Internal CI is green: lib has 10 test suites passing including the 3 public-API contract tests; runtime has 3 test suites passing (38 tests). Phase 3 spans commits `1e2bdaa1`..`a5b2c722`. **US1 is independently shippable contingent on T027/T028 being completed.**
