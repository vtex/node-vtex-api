...
import { IOContext, ParamsContext, RecorderState, ServiceContext } from '../../typings'
...
const formatMillis = (millis: number) =>
  millis >= 500
    ? chalk.red(millis.toString())
    : millis >= 200
    ? chalk.yellow(millis.toString())
    : chalk.green(millis.toString())

const log = <T extends IOClients, U extends RecorderState, V extends ParamsContext>(
  {
    vtex: {
      account,
      workspace,
  ...
  `${formatDate(new Date())}\t${pid}\t${account}/${workspace}:${id}\t${formatStatus(
    status
  )}\t${method}\t${path}\t${formatMillis(millis)} ms`
