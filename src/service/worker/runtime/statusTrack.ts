...
export const isStatusTrack = (message: any): message is typeof STATUS_TRACK => message === STATUS_TRACK
...
export const broadcastStatusTrack = () => Object.values(cluster.workers).forEach(worker => worker?.send(STATUS_TRACK))

const logStatus = (status: EnvMetric) =>
  console.log(
    JSON.stringify({
      __VTEX_IO_LOG: true,
      account: ACCOUNT,
