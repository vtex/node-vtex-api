...
const pickCacheControlHints = (response: GraphQLResponse) =>
  path<CacheControlHintsFormat>(['extensions', 'cacheControl', 'hints'], response)
...
const isPublicEndpoint = ({ request: { headers } }: GraphQLServiceContext) =>
  headers.origin ? false : test(publicRegExp, headers['x-forwarded-host'] || '')
...
export const cacheControl = (response: GraphQLResponse, ctx: GraphQLServiceContext) => {
  ...
  return {
    maxAge:
      age === 0 || isPublicEndpoint(ctx) || !production ? 'no-cache, no-store' : `max-age=${age}`,
