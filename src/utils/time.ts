...
export const hrToMillis = ([seconds, nanoseconds]: [number, number]) => Math.round(seconds * 1e3 + nanoseconds / 1e6)
...
export const reduceHrToNano = reduce((acc: number, hr: [number, number]) => acc + hrToNano(hr), 0 as number)
...
export const reduceTimings = (timingsObj: Record<string, string>) =>
  compose<Record<string, string>, Array<[string, string]>, string>(
...
function recordTimings(
  ...
  const elapsed: [number, number] = [end[0] - timings.total[0], end[1] - timings.total[1]]
...
export function timer<T extends IOClients, U extends RecorderState, V extends ParamsContext>(
  ...
  if (!middleware.name) {
    logOnceToDevConsole(
      `Please use a named function as handler for better metrics. ${middleware.toString()}`,
      LogLevel.Warn
    )
  }
