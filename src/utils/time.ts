...
export const shrinkTimings = (name: string) =>
  name.replace(/graphql/g, 'gql').replace(/server/g, 'srv')

type TimingFormat = ReturnType<typeof parseTimingName>

export const formatTimingName = ({ hopNumber, target, source }: TimingFormat) =>
  `${Number.isNaN(hopNumber as any) ? '' : hopNumber}.${source ||
    ''}#${target || ''}`
