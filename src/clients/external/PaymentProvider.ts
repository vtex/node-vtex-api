...
const routes = {
  callback: (transactionId: string, paymentId: string) => `${routes.payment(transactionId, paymentId)}/notification`,
  inbound: (transactionId: string, paymentId: string, action: string) =>
    `${routes.payment(transactionId, paymentId)}/inbound-request/${action}`,
  payment: (transactionId: string, paymentId: string) => `/transactions/${transactionId}/payments/${paymentId}`,
}
...
export class PaymentProvider extends ExternalClient {
  ...
  public callback = (transactionId: string, paymentId: string, callback: AuthorizationCallback) =>
    this.http.post<unknown>(routes.callback(transactionId, paymentId), callback, {
      metric: 'gateway-callback',
    })

  public inbound = <TRequest, TResponse>(transactionId: string, paymentId: string, action: string, payload: TRequest) =>
    this.http.post<TResponse>(routes.inbound(transactionId, paymentId, action), payload, {
