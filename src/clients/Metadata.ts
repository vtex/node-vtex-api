...
const routes = {
  ...
  MetadataKey: (bucket: string, key: string) =>
    `${routes.Metadata(bucket)}/${key}`,
}
...
export class Metadata extends InfraClient {
  public constructor(context: IOContext, options?: InstanceOptions) {
    ...
    if (runningAppName === '') {
      throw new Error(
        `Invalid path to access Metadata. Variable VTEX_APP_ID is not available.`
      )
    }
  }
  ...
  public list = (
    ...
    return this.http.get<MetadataEntryList>(routes.Metadata(bucket), {
      params: query,
      metric,
    })
  }
