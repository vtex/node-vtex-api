...
const routes = {
  ...
  Promote: (account: string) =>
    `${routes.Workspace(account, DEFAULT_WORKSPACE)}/_promote`,
...
export class Workspaces extends InfraClient {
  ...
  public list = (account: string) => {
    return this.http.get<WorkspaceMetadata[]>(routes.Account(account), {
      metric: 'workspaces-list',
    })
  }

  public get = (account: string, workspace: string) => {
    return this.http.get<WorkspaceMetadata>(
      routes.Workspace(account, workspace),
      { metric: 'workspaces-get' }
    )
  }
  ...
  public promote = (account: string, workspace: string) => {
    return this.http.put(
      routes.Promote(account),
      { workspace },
      { metric: 'workspaces-promote' }
    )
  }
}
