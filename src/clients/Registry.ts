...
export class Registry extends InfraClient {
  ...
  public getAppJSON = <T extends object | null>(
    ...
    return this.http.get<T>(routes.AppFile(app, version, path), configs)
  }
  ...
  public unpackAppBundle = (
    ...
    return this.getAppBundle(
      app,
      version,
      bundlePath,
      generatePackageJson
    ).then(stream => stream.pipe(createGunzip()).pipe(extract(unpackPath)))
  }
