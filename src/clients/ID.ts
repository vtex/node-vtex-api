...
export class ID extends ExternalClient {
  public constructor(context: IOContext, opts?: InstanceOptions) {
  ...
  public getEmailCodeAuthenticationToken = (
    token: string,
    email: string,
    code: string
  ) => {
    ...
    return this.http.get<AuthenticationResponse>(routes.VALIDATE, {
      params,
      metric,
    })
  }

  public getPasswordAuthenticationToken = (
    token: string,
    email: string,
    password: string
  ) => {
    ...
    return this.http.get<AuthenticationResponse>(routes.VALIDATE_CLASSIC, {
      params,
      metric,
    })
  }
