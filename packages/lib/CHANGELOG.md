...
## [8.0.0-0] - Phase 3 carve-out (workstream C)

This is the first MAJOR release after the monorepo split (feature
`001-split-node-vtex-api-library`). It removes runtime-internal symbols
from `@vtex/api`'s public surface and ships them inside a new internal
package `@vtex/api-runtime`, consumed only by `service-runtime-node`.
Consumer apps that imported `@vtex/api` for clients, errors, logger,
metrics, tracing helpers, GraphQL schema directives, recorder, and the
`Service` class require **no code change** — those symbols are
unchanged. The single removed symbol is documented below.

### Removed

- **`startApp`** — The Koa app bootstrap function that booted the master
  + worker cluster, initialized telemetry, and called `app.listen()`.
  Belongs to the platform runtime, not the consumer-app SDK. Consumer
  apps that imported `startApp` directly would have been double-booting
  the runtime. **Use `service-runtime-node`** — it imports `startApp`
  from `@vtex/api-runtime` (the new private package) and is the
  documented entry point for VTEX IO Node services.

### Internal (NOT public, marked `@internal`)

The following 14 named re-exports were added to `packages/lib/src/index.ts`
as a TRANSITIONAL aid for the carve-out. They expose runtime-internal
symbols that `@vtex/api-runtime` still imports cross-package. They are
NOT documented public API; consumer apps must not depend on them. A
future minor will either trim them entirely or promote the
cross-cutting ones (e.g. `HttpAgentSingleton`, where a single
process-wide instance is required) to a documented `@vtex/api/internals`
subpath:

- `HttpAgentSingleton` (used by runtime statusTrack)
- `MetricsLogger` (used by runtime builtIn middleware)
- `statusLabel` (used by runtime http timings middleware)
- `cancel` (used by runtime compose util)
- `UserLandTracer` (the class; used by runtime tracing middleware)
- `AppTags`, `CustomHttpTags`, `OpentracingTags`, `VTEXIncomingRequestTags`
  (used by runtime tracing middleware)
- `RuntimeLogEvents`, `RuntimeLogFields` (used by runtime tracing
  middleware)
- `cloneAndSanitizeHeaders` (used by runtime tracing middleware)
- `IOMessage`, `BindingHeader`, `TenantHeader` (referenced by runtime's
  type imports)

### Changed

- The repository is now a Yarn 1 workspaces monorepo. The `@vtex/api`
  npm package lives at `packages/lib/`. A sibling package
  `@vtex/api-runtime` lives at `packages/runtime/` and is shipped only
  inside `service-runtime-node`. See the root `README.md` for the
  monorepo layout.
- `engines.node` continues to advertise `>= 8`; CI publish runs on
  Node 22.

### Known coupling debt (not blocking 8.0.0)

- SC-003 ("zero bytes of Koa/server/middleware in consumer bundles")
  is partially met. The PUBLIC SURFACE (`index.d.ts` re-exports) is
  correctly trimmed, but BYTES of some Koa-touching middleware files
  (`http/middlewares/{settings,requestStats,timings}.ts`,
  `service/tracing/`, `service/metrics/`,
  `service/worker/runtime/statusTrack.ts`,
  `service/worker/listeners.ts`) remain in `@vtex/api` because
  surviving public classes (`MetricsAccumulator`, `DiagnosticsMetrics`,
  `Logger`, `schemaDirectives`, `AppGraphQLClient`, `UserLandTracer`)
  still import them. Breaking these couplings requires non-trivial
  refactoring of multiple public classes and is tracked as Phase 4+
  work. The trade-off was accepted in commit `a6eb0e82` (T024).
- The runtime-only constants `MAX_WORKERS`, `UP_SIGNAL`, `PID`,
  `INSPECT_DEBUGGER_PORT`, `HTTP_SERVER_PORT` remain reachable via
  `@vtex/api` because `index.ts` wildcards the shared `constants.ts`
  file. Splitting `constants.ts` into lib-only and runtime-only halves
  is feasible but out of Phase 3 scope.
